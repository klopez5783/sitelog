...
const CATEGORIES = [
  { id: "contracts", name: "Contracts", description: "Agreements and signed contracts", emoji: "📝" },
  { id: "permits",   name: "Permits",   description: "Building permits and approvals",  emoji: "🏛️" },
  { id: "plans",     name: "Plans",     description: "Blueprints and drawings",          emoji: "📐" },
  { id: "photos",    name: "Photos",    description: "Site photos and progress images",  emoji: "📸" },
];
  ...
  return (
    <ScrollView className="flex-1 px-4 pt-4" showsVerticalScrollIndicator={false}>
      <TouchableOpacity
        ...
        onPress={() =>
          router.push(`/job/document/add?jobId=${jobId}${activeCategory ? `&category=${activeCategory}` : ""}`)
        }
      >
      ...
      {documents.length === 0 ? (
        <View className="gap-3">
          {CATEGORIES.map((cat) => (
            <TouchableOpacity
              key={cat.id}
              onPress={() => router.push(`/job/document/add?jobId=${jobId}&category=${cat.id}`)}
        ...
        filteredDocs.map((doc) => (
          <TouchableOpacity
            key={doc.id}
            onPress={() => router.push(`/job/document/${doc.id}?jobId=${jobId}`)}
            activeOpacity={0.7}
          >
            <Card>
              <View className="flex-row items-center gap-3">
                <View className="w-10 h-10 bg-orange-100 rounded-xl items-center justify-center">
                  <Text className="text-lg">{getCategoryEmoji(doc.category)}</Text>
                </View>
                <View className="flex-1">
