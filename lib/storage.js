...
// ── Add to lib/storage.js ─────────────────────────────────

export const uploadDocumentFile = async (companyId, jobId, uri, originalName, mimeType) => {
  const ext = originalName.split(".").pop() || "bin";
  const filename = `${Date.now()}_${Math.random().toString(36).slice(2)}.${ext}`;
  const storagePath = `companies/${companyId}/jobs/${jobId}/documents/${filename}`;
  const storageRef = ref(storage, storagePath);

  const blob = await uriToBlob(uri);  // reuse existing uriToBlob from storage.js
